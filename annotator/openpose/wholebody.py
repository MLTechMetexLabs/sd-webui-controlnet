...
class Wholebody:
    def __init__(self, onnx_det: str, onnx_pose: str):
        ...
        device = 'cuda'
        backend = cv2.dnn.DNN_BACKEND_OPENCV if device == 'cuda' else cv2.dnn.DNN_BACKEND_CUDA
        # You need to manually build OpenCV through cmake to work with your GPU.
        providers = cv2.dnn.DNN_TARGET_CPU if device == 'cuda' else cv2.dnn.DNN_TARGET_CUDA
